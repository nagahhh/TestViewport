================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : ViewportTest プロジェクトの概要
===============================================================================

この ViewportTest アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、ViewportTest アプリケーションを構成する各ファイルの内容の概要が含まれています。

ViewportTest.vcxproj
...
ViewportTest.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CViewportTestApp アプリケーション クラスを宣言します。

ViewportTest.cpp
    これはメインのアプリケーション ソース ファイルで、CViewportTestApp アプリケーション クラスが含まれています。
...
res\ViewportTest.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル ViewportTest.rc にインクルードされます。

res\ViewportTest.rc2
...
ViewportTestDoc.h, ViewportTestDoc.cpp - ドキュメント
    これらのファイルには、CViewportTestDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CViewportTestDoc::Serialize を使用)。

ViewportTestView.h, ViewportTestView.cpp - ドキュメントのビュー
    これらのファイルには、CViewportTestView クラスが含まれます。
    CViewportTestView オブジェクトは CViewportTestDoc オブジェクトを表示するために使用されます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、ViewportTest.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

ViewportTest.manifest
